/**
 * Rutas de manejo de documentos PDF para documentación de embarcación en el ERP Megui.
 * Permite subir PDFs generados a partir de fotos de documentos de embarcación.
 * Sigue el patrón establecido en documentacion.personal.routes.js.
...
// Carpeta base para documentación de embarcación
const DOCUMENTACION_EMBARCACION_DIR = path.join(process.cwd(), 'uploads', 'documentacion-embarcacion');
if (!fs.existsSync(DOCUMENTACION_EMBARCACION_DIR)) {
  fs.mkdirSync(DOCUMENTACION_EMBARCACION_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de documentación de embarcación
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = DOCUMENTACION_EMBARCACION_DIR;
...
/**
 * POST /api/documentacion-embarcacion/upload
 * Sube un PDF generado a partir de fotos de documentos de embarcación.
 * Retorna la URL relativa para guardar en DocumentacionEmbarcacion.urlDocPdf
 */
router.post('/upload', autenticarJWT, (req, res, next) => {
  ...
  try {
    const { embarcacionId, documentacionEmbarcacionId } = req.body;
    ...
    // Si se proporciona documentacionEmbarcacionId, actualiza el registro
    if (documentacionEmbarcacionId) {
      await prisma.documentacionEmbarcacion.update({
        where: { id: BigInt(documentacionEmbarcacionId) },
        data: { urlDocPdf: rutaRelativa }
      });
    }
    ...
    res.json({ 
      ...
      mensaje: 'Documento de embarcación subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP DOCUMENTACION EMBARCACION] Error al subir documento:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar el documento de embarcación.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/documentacion-embarcacion/archivo/*
 * Sirve archivos PDF de documentación de embarcación con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'documentacion-embarcacion', rutaArchivo);
    ...
    console.error('Error sirviendo documento de embarcación:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
