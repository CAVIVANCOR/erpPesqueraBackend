...
// Carpeta base para PDFs de liquidaciones de entregas a rendir
const ENTREGAS_RENDIR_DIR = path.join(process.cwd(), 'uploads', 'entregas-rendir');
if (!fs.existsSync(ENTREGAS_RENDIR_DIR)) {
  fs.mkdirSync(ENTREGAS_RENDIR_DIR, { recursive: true });
}
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = ENTREGAS_RENDIR_DIR;
...
/**
 * POST /api/entregas-a-rendir/upload-pdf
 * Sube un PDF generado de liquidación de entrega a rendir.
 * Retorna la URL relativa para guardar en EntregaARendir.urlLiquidacionPdf
 */
router.post('/upload-pdf', autenticarJWT, (req, res, next) => {
  ...
  try {
    ...
    if (entregaId) {
      await prisma.entregaARendir.update({
    ...
    console.error('[ERP ENTREGA RENDIR] Error al subir PDF:', error);
    res.status(500).json({ 
...
/**
 * GET /api/entregas-a-rendir/archivo/*
...
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'entregas-rendir', rutaArchivo);
