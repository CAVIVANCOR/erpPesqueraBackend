/**
 * Rutas de manejo de documentos PDF para confirmaciones de acciones previas CONSUMO en el ERP Megui.
 * Permite subir PDFs generados a partir de fotos de confirmaciones de acciones previas de pesca consumo.
 * Sigue el patrón establecido en confirmaciones.acciones.previas.routes.js pero adaptado para DetAccionesPreviasFaenaConsumo.
...
// Carpeta base para confirmaciones de acciones previas consumo
const CONFIRMACIONES_DIR = path.join(process.cwd(), 'uploads', 'confirmaciones-acciones-previas-consumo');
if (!fs.existsSync(CONFIRMACIONES_DIR)) {
  fs.mkdirSync(CONFIRMACIONES_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de confirmaciones de acciones previas consumo
...
/**
 * POST /api/confirmaciones-acciones-previas-consumo/upload
 * Sube un PDF generado a partir de fotos de confirmaciones de acciones previas consumo.
 * Retorna la URL relativa para guardar en DetAccionesPreviasFaenaConsumo.urlConfirmaAccionPdf
 */
router.post('/upload', autenticarJWT, (req, res, next) => {
  ...
  try {
    const { accionPreviaId, faenaPescaConsumoId, detAccionesPreviasFaenaConsumoId } = req.body;
    ...
    // Si se proporciona detAccionesPreviasFaenaConsumoId, actualiza el registro
    if (detAccionesPreviasFaenaConsumoId) {
      await prisma.detAccionesPreviasFaenaConsumo.update({
        where: { id: BigInt(detAccionesPreviasFaenaConsumoId) },
    ...
    res.json({ 
      ...
      mensaje: 'Confirmación de acción previa consumo subida exitosamente.'
    });

  } catch (error) {
    console.error('[ERP CONFIRMACIONES ACCIONES PREVIAS CONSUMO] Error al subir documento:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar la confirmación de acción previa consumo.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/confirmaciones-acciones-previas-consumo/archivo/*
 * Sirve archivos PDF de confirmaciones de acciones previas consumo con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'confirmaciones-acciones-previas-consumo', rutaArchivo);
    ...
    console.error('Error sirviendo confirmación acción previa consumo:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
