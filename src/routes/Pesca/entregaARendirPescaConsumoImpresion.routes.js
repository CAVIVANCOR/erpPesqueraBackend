/**
 * Rutas de manejo de PDFs de liquidación para EntregaARendirPescaConsumo en el ERP Megui.
...
const ENTREGAS_RENDIR_CONSUMO_DIR = path.join(process.cwd(), 'uploads', 'entregas-rendir-consumo');
if (!fs.existsSync(ENTREGAS_RENDIR_CONSUMO_DIR)) {
  fs.mkdirSync(ENTREGAS_RENDIR_CONSUMO_DIR, { recursive: true });
}
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = ENTREGAS_RENDIR_CONSUMO_DIR;
  ...
  filename: function (req, file, cb) {
    try {
      ...
      if (entregaId) {
        const fileName = `LIQUIDACION-PESCA-CONSUMO-${entregaId}${ext}`;
        cb(null, fileName);
      } else {
        const now = new Date();
...
router.post('/upload-pdf', upload.single('file'), async (req, res) => {
  try {
    ...
    const rutaRelativa = path.join(
      '/uploads/entregas-rendir-consumo',
      String(year),
      month,
      req.file.filename
    ).replace(/\\/g, '/');
    ...
    if (entregaId) {
      await prisma.entregaARendirPescaConsumo.update({
    ...
    res.json({ 
      ...
      mensaje: 'PDF de liquidación de pesca consumo subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP ENTREGA RENDIR PESCA CONSUMO] Error al subir PDF:', error);
    res.status(500).json({ 
...
router.get('/archivo/*', async (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'entregas-rendir-consumo', rutaArchivo);
