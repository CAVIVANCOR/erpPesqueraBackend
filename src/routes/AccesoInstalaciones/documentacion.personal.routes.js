/**
 * Rutas de manejo de documentos PDF para documentación personal en el ERP Megui.
 * Permite subir PDFs generados a partir de fotos de documentos de personal.
 * Sigue el patrón establecido en confirmaciones.acciones.previas.routes.js.
...
// Carpeta base para documentación personal
const DOCUMENTACION_PERSONAL_DIR = path.join(process.cwd(), 'uploads', 'documentacion-personal');
if (!fs.existsSync(DOCUMENTACION_PERSONAL_DIR)) {
  fs.mkdirSync(DOCUMENTACION_PERSONAL_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de documentación personal
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = DOCUMENTACION_PERSONAL_DIR;
...
/**
 * POST /api/documentacion-personal/upload
 * Sube un PDF generado a partir de fotos de documentos de personal.
 * Retorna la URL relativa para guardar en DocumentacionPersonal.urlDocPdf
 */
router.post('/upload', autenticarJWT, (req, res, next) => {
  ...
  try {
    const { personalId, documentoPescaId, documentacionPersonalId } = req.body;
    ...
    // Si se proporciona documentacionPersonalId, actualiza el registro
    if (documentacionPersonalId) {
      await prisma.documentacionPersonal.update({
        where: { id: BigInt(documentacionPersonalId) },
    ...
    res.json({ 
      ...
      mensaje: 'Documento de personal subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP DOCUMENTACION PERSONAL] Error al subir documento:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar el documento de personal.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/documentacion-personal/archivo/*
 * Sirve archivos PDF de documentación personal con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'documentacion-personal', rutaArchivo);
    ...
    console.error('Error sirviendo documento personal:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
