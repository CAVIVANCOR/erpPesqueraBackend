...
// Carpeta base para documentos requeridos de cotización de ventas
const DOCUMENTOS_REQUERIDOS_DIR = path.join(process.cwd(), 'uploads', 'documentos-requeridos-ventas');
if (!fs.existsSync(DOCUMENTOS_REQUERIDOS_DIR)) {
  fs.mkdirSync(DOCUMENTOS_REQUERIDOS_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de documentos requeridos
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = DOCUMENTOS_REQUERIDOS_DIR;
...
router.get('/cotizacion/:cotizacionVentaId', detDocsReqCotizaVentasController.listarPorCotizacion);
...
/**
 * POST /api/det-docs-req-cotiza-ventas/upload-documento
 * Sube un PDF de documento requerido para cotización de ventas.
 * Retorna la URL relativa para guardar en DetDocsReqCotizaVentas.urlDocumento
 */
router.post('/upload-documento', autenticarJWT, (req, res, next) => {
  ...
  try {
    const { detDocsReqCotizaVentasId } = req.body;
    ...
    // Si se proporciona detDocsReqCotizaVentasId, actualiza el registro
    if (detDocsReqCotizaVentasId) {
      await prisma.detDocsReqCotizaVentas.update({
        where: { id: BigInt(detDocsReqCotizaVentasId) },
    ...
    res.json({ 
      ...
      mensaje: 'Documento requerido subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP DOCS REQUERIDOS VENTAS] Error al subir documento:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar el documento requerido.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/det-docs-req-cotiza-ventas/archivo/*
 * Sirve archivos PDF de documentos requeridos con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'documentos-requeridos-ventas', rutaArchivo);
    ...
    console.error('Error sirviendo documento requerido:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
