...
// Carpeta base para PDFs de cotizaciones de ventas
const COTIZACIONES_VENTAS_DIR = path.join(process.cwd(), 'uploads', 'cotizaciones-ventas');
if (!fs.existsSync(COTIZACIONES_VENTAS_DIR)) {
  fs.mkdirSync(COTIZACIONES_VENTAS_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de cotizaciones
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = COTIZACIONES_VENTAS_DIR;
...
/**
 * POST /api/cotizacion-ventas/upload-pdf
 * Sube un PDF generado de la cotización de ventas.
 * Retorna la URL relativa para guardar en CotizacionVentas.urlCotizacionVentaPdf
 */
router.post('/upload-pdf', autenticarJWT, (req, res, next) => {
  ...
  try {
    ...
    // Si se proporciona cotizacionId, actualiza el registro
    if (cotizacionId) {
    ...
    res.json({ 
      ...
      urlCotizacionVentaPdf: rutaRelativa,
      url: rutaRelativa,
      nombreArchivo: req.file.filename,
      mensaje: 'PDF de la cotización de ventas subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP COTIZACION VENTAS] Error al subir PDF:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar el PDF de la cotización de ventas.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/cotizacion-ventas/archivo/*
 * Sirve archivos PDF de cotizaciones con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'cotizaciones-ventas', rutaArchivo);
    
    // Verificar que el archivo existe
    if (!fs.existsSync(rutaCompleta)) {
      return res.status(404).json({
        error: 'PDF de la cotización no encontrado',
    ...
    console.error('Error sirviendo PDF de cotización de ventas:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
