/**
 * Rutas de manejo de PDFs de liquidación para EntregaARendirPVentas en el ERP Megui.
...
const ENTREGAS_RENDIR_VENTAS_DIR = path.join(process.cwd(), 'uploads', 'entregas-rendir-ventas');
if (!fs.existsSync(ENTREGAS_RENDIR_VENTAS_DIR)) {
  fs.mkdirSync(ENTREGAS_RENDIR_VENTAS_DIR, { recursive: true });
}
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = ENTREGAS_RENDIR_VENTAS_DIR;
  ...
  filename: function (req, file, cb) {
    try {
      ...
      if (entregaId) {
        const fileName = `LIQUIDACION-VENTAS-${entregaId}${ext}`;
        cb(null, fileName);
      } else {
        const now = new Date();
...
router.post('/upload-pdf', upload.single('file'), async (req, res) => {
  try {
    ...
    const rutaRelativa = path.join(
      '/uploads/entregas-rendir-ventas',
      String(year),
      month,
      req.file.filename
    ).replace(/\\/g, '/');
    ...
    if (entregaId) {
      await prisma.entregaARendirPVentas.update({
    ...
    res.json({ 
      ...
      mensaje: 'PDF de liquidación de ventas subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP ENTREGA RENDIR VENTAS] Error al subir PDF:', error);
    res.status(500).json({ 
...
router.get('/archivo/*', async (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'entregas-rendir-ventas', rutaArchivo);
