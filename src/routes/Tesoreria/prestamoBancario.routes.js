...
const PRESTAMOS_DOCS_DIR = path.join(process.cwd(), 'uploads', 'prestamos-bancarios');
...
router.post('/upload', autenticarJWT, checkPermission('prestamoBancario', 'crear'), (req, res, next) => {
  ...
  try {
    ...
    console.error('[ERP PRESTAMOS BANCARIOS] Error al subir documento:', error);
    res.status(500).json({ 
...
router.get('/archivo/*', autenticarJWT, checkPermission('prestamoBancario', 'ver'), (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'prestamos-bancarios', rutaArchivo);
