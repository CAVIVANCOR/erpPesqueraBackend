...
// Carpeta base para PDFs de liquidaciones de entregas a rendir de movimientos de almacén
const ENTREGAS_RENDIR_MOV_ALMACEN_DIR = path.join(process.cwd(), 'uploads', 'entregas-rendir-mov-almacen');
if (!fs.existsSync(ENTREGAS_RENDIR_MOV_ALMACEN_DIR)) {
  fs.mkdirSync(ENTREGAS_RENDIR_MOV_ALMACEN_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de liquidaciones
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = ENTREGAS_RENDIR_MOV_ALMACEN_DIR;
...
/**
 * POST /api/entregas-rendir-mov-almacen/upload-pdf
 * Sube un PDF generado de liquidación de entrega a rendir de movimiento de almacén.
 * Retorna la URL relativa para guardar en EntregaARendirMovAlmacen.urlLiquidacionPdf
 */
router.post('/upload-pdf', autenticarJWT, (req, res, next) => {
  ...
  try {
    ...
    const rutaRelativa = path.join(
      '/uploads/entregas-rendir-mov-almacen',
      String(year),
      month,
      req.file.filename
    ).replace(/\\/g, '/'); // Normaliza para Windows/Linux

    // Si se proporciona entregaId, actualiza el registro
    if (entregaId) {
      await prisma.entregaARendirMovAlmacen.update({
    ...
    res.json({ 
      ...
      mensaje: 'PDF de liquidación subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP ENTREGA RENDIR MOV ALMACEN] Error al subir PDF:', error);
    res.status(500).json({ 
...
/**
 * GET /api/entregas-rendir-mov-almacen/archivo/*
 * Sirve archivos PDF de liquidaciones con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'entregas-rendir-mov-almacen', rutaArchivo);
