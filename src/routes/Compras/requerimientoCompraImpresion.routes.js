...
// Carpeta base para PDFs de requerimientos de compra
const REQUERIMIENTOS_COMPRA_DIR = path.join(process.cwd(), 'uploads', 'requerimientos-compra');
if (!fs.existsSync(REQUERIMIENTOS_COMPRA_DIR)) {
  fs.mkdirSync(REQUERIMIENTOS_COMPRA_DIR, { recursive: true });
}

// Configuración de Multer para guardar PDFs de requerimientos
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    try {
      ...
      const baseDir = REQUERIMIENTOS_COMPRA_DIR;
...
/**
 * POST /api/requerimiento-compra/upload-pdf
 * Sube un PDF generado a partir de fotos o PDF directo del requerimiento de compra.
 * Retorna la URL relativa para guardar en RequerimientoCompra.urlReqCompraPdf
 */
router.post('/upload-pdf', autenticarJWT, (req, res, next) => {
  ...
  try {
    ...
    // Si se proporciona requerimientoId, actualiza el registro
    if (requerimientoId) {
      await prisma.requerimientoCompra.update({
        where: { id: BigInt(requerimientoId) },
        data: { urlReqCompraPdf: rutaRelativa }
      });
    }
    ...
    res.json({ 
      ...
      urlReqCompraPdf: rutaRelativa,
      url: rutaRelativa,
      nombreArchivo: req.file.filename,
      mensaje: 'PDF del requerimiento de compra subido exitosamente.'
    });

  } catch (error) {
    console.error('[ERP REQUERIMIENTO COMPRA] Error al subir PDF:', error);
    res.status(500).json({ 
      error: 'Error interno al guardar el PDF del requerimiento de compra.',
      codigo: 'ERR_SERVIDOR'
    });
  }
});
...
/**
 * GET /api/requerimiento-compra/archivo/*
 * Sirve archivos PDF de requerimientos con autenticación JWT
 */
router.get('/archivo/*', autenticarJWT, (req, res) => {
  try {
    ...
    const rutaCompleta = path.join(process.cwd(), 'uploads', 'requerimientos-compra', rutaArchivo);
    
    // Verificar que el archivo existe
    if (!fs.existsSync(rutaCompleta)) {
      return res.status(404).json({
        error: 'PDF del requerimiento no encontrado',
    ...
    console.error('Error sirviendo PDF de requerimiento de compra:', error);
    res.status(500).json({
      error: 'Error interno del servidor',
      codigo: 'ERR_SERVIDOR',
