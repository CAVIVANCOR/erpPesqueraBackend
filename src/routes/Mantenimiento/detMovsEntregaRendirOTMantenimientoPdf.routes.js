// src/routes/Mantenimiento/detMovsEntregaRendirOTMantenimientoPdf.routes.js
// Rutas para upload y serving de PDFs de Movimientos de Entrega a Rendir de OT Mantenimiento
// Patrón profesional replicado EXACTAMENTE de detMovsEntregaRendirMovAlmacenPdf.routes.js
...
const storageComprobante = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, '../../../uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-movimiento');
  ...
  filename: (req, file, cb) => {
    ...
    const filename = `comprobante-mov-ot-mantenimiento-${movimientoId || 'temp'}-${timestamp}${extension}`;
    cb(null, filename);
  }
});
...
const storageOperacion = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(__dirname, '../../../uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-operacion');
  ...
  filename: (req, file, cb) => {
    ...
    const filename = `comprobante-operacion-ot-mantenimiento-${movimientoId || 'temp'}-${timestamp}${extension}`;
    cb(null, filename);
  }
});
 ...
 * MISMO PATRÓN QUE detMovsEntregaRendirMovAlmacenPdf.routes.js
 */
router.post('/upload-pdf-comprobante', uploadComprobante.single('documento'), async (req, res) => {
  try {
    ...
    const rutaRelativa = `/uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-movimiento/${req.file.filename}`;
    
    // Si se proporciona movimientoId, actualiza el registro
    const movimientoId = req.body.movimientoId;
    if (movimientoId) {
      try {
        await prisma.detMovsEntregaRendirOTMantenimiento.update({
...
router.post('/upload-pdf-operacion', uploadOperacion.single('documento'), async (req, res) => {
  try {
    ...
    const rutaRelativa = `/uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-operacion/${req.file.filename}`;
...
router.get('/archivo-comprobante/:filename', (req, res) => {
  try {
    ...
    const filePath = path.join(__dirname, '../../../uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-movimiento', filename);
...
router.get('/archivo-operacion/:filename', (req, res) => {
  try {
    const { filename } = req.params;
    const filePath = path.join(__dirname, '../../../uploads/det-movs-entrega-rendir-ot-mantenimiento/comprobantes-operacion', filename);
