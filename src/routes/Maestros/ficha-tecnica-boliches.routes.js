/**
 * Rutas de manejo de ficha técnica para Boliches Red en el ERP Megui.
...
// Carpeta destino para fichas técnicas de boliches red (ruta absoluta desde la raíz del proyecto)
const FICHA_TECNICA_DIR = path.join(process.cwd(), 'uploads', 'fichas-tecnicas-boliches');
...
/**
 * POST /api/ficha-tecnica-boliches/upload
...
router.post('/upload', upload.single('fichaTecnica'), async (req, res) => {
  try {
    ...
    const bolicheRed = await prisma.bolicheRed.update({
      ...
      data: { urlBolicheRedPdf: `/uploads/fichas-tecnicas-boliches/${newFilename}` }
    });

    res.json({
      message: 'Ficha técnica de boliche red subida correctamente.',
      urlBolicheRedPdf: `/uploads/fichas-tecnicas-boliches/${newFilename}`,
...
/**
 * GET /api/ficha-tecnica-boliches/archivo/:filename
