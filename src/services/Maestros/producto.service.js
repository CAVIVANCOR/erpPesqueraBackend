...
function normalizarDatosProducto(data) {
  ...
  // ✅ LÓGICA CRÍTICA: Si no se envía unidadMedidaComercialId, igualar a unidadMedidaId
  const unidadMedidaComercialId = data.unidadMedidaComercialId !== undefined &&
    data.unidadMedidaComercialId !== null &&
    data.unidadMedidaComercialId !== 0
    ? data.unidadMedidaComercialId  // Respetar valor ingresado
    : data.unidadMedidaId;           // Fallback a unidadMedidaId
  ...
  return {
    ...
    descripcionEspanolExportacion: emptyToNull(toUpperCaseSafe(data.descripcionEspanolExportacion)),
    descripcionInglesExportacion: emptyToNull(toUpperCaseSafe(data.descripcionInglesExportacion)),
    ...
    // ✅ ASIGNAR unidadMedidaComercialId con lógica de fallback
    unidadMedidaComercialId: unidadMedidaComercialId,
  };
}
...
async function validarProducto(data, excluirId = null) {
  ...
  if (data.unidadMedidaComercialId !== undefined &&
    data.unidadMedidaComercialId !== null &&
    data.unidadMedidaComercialId !== 0) {
    const unidadComercial = await prisma.unidadMedida.findUnique({
      where: { id: data.unidadMedidaComercialId }
    });
    if (!unidadComercial) {
      throw new ValidationError('Unidad de medida comercial no existente.');
    }
  }
